	...
	<el-dialog :title="type == 'add' ? '添加分类' : '修改分类'" :close-on-click-modal="false" :visible.sync="visible"
		...
		<el-form :model="ruleForm" :rules="rules" ref="formRef" label-width="100px">
			...
			<el-form-item label="父级分类:" >
				...
				<el-breadcrumb separator="/" v-else style="line-height: 40px;">	
			...
			<el-form-item label="排序值" prop="category_rank">
				<el-input type="number" max='200' v-model="ruleForm.category_rank"></el-input>
			</el-form-item>
		</el-form>
		<template #footer>
...
<script>
	export default {
		...
		methods: {
			...
			async getDetail(id) {
				await this.$axios.get(`/categories/${id}`).then(res => {
					this.ruleForm = {
							...
							category_rank: res.data.categoryRank,
							is_ok: res.data.isOk,
						},
			...
			//编辑时，通过id获取父级分类id数组
			async getCatId(id) {
				await this.$axios.get('/getParentId', {
					params: {
						id: id
					},
				}).then(res => {
