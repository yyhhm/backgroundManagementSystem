<template>
	<el-card>
		<template #header>
			<div class="header">
				<el-button type="danger" size="small" icon="el-icon-delete" @click="handleDelete()">删除评价</el-button>
			</div>
		</template>
		<el-table ref="multipleTable" :data="tableData"  tooltip-effect="dark" style="width: 100%"
			...
			<el-table-column label="商品信息" width="130">
				<template #default="scope">
					<img style="width: 100px; height: 100px;" :src="scope.row.orderItem.bookCoverImg.split('#')[0]"
						alt="主图">
					
				</template>

			</el-table-column>
			<el-table-column width="200" prop="orderItem.bookName" >
			
			</el-table-column>
			<el-table-column width="100" prop="users.userId" label="用户Id">
				...
				<template #default="scope">
					<div v-if="scope.row.appraiseContact.length>0">
						<el-popover placement="top-start" width="300" trigger="hover"
							...
							<span slot="reference"
								v-if="scope.row.appraiseContact.length <= 10">{{scope.row.appraiseContact}}</span>
							<span slot="reference"
								v-if="scope.row.appraiseContact.length > 10">{{scope.row.appraiseContact.substr(0, 10) + "..."}}</span>
						</el-popover>
					</div>
		...
		<div class="block">
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
				:current-page="currentPage" :page-sizes="[8,12,16,20]" :page-size="pageSize"
